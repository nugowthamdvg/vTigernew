...
public class TestScript2 extends Base
{
	...
	@Test
	public void createContactsTest1() throws InterruptedException
	{
		...
		cCon.salutation(lib.getSpecifiedDataFromExcelFile(lib.getDataFromPrpoertyFile("Sheet2"), 1, 0));
		Reporter.log("Salutation is Selected",true);
		
		cCon.getContactFN().sendKeys(lib.getSpecifiedDataFromExcelFile(lib.getDataFromPrpoertyFile("Sheet2"),1, 1));
		Reporter.log("Contact First Name is Entered",true);
		
		cCon.getContactLN().sendKeys(lib.getSpecifiedDataFromExcelFile(lib.getDataFromPrpoertyFile("Sheet2"), 1, 2));
		Reporter.log("Contact Last Name is Entered",true);
		
		cCon.getContactMobile().sendKeys(lib.getSpecifiedDataFromExcelFile(lib.getDataFromPrpoertyFile("Sheet2"), 1, 3));
		Reporter.log("Contact Mobile number is Entered",true);
		
		cCon.getContactPO().sendKeys(lib.getSpecifiedDataFromExcelFile(lib.getDataFromPrpoertyFile("Sheet2"),1, 4));
